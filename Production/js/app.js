      ...
      class Point {
        ...
        addToScene(scene) {
          ...
          this.label.renderOrder = 11; // Les labels doivent être au-dessus des points
      ...
      class Vector {
        ...
        addToScene(scene) {
          ...
          // --- AJOUT ---
          if (this.label) {
            this.label.renderOrder = 11; // Pour être au-dessus des autres objets
            scene.add(this.label);
          }
        }
      ...
      const SWIPE_EDGE_ZONE = 40; // Zone de détection sur le bord gauche
      ...
      function init() {
        ...
        // Gestion de la case à cocher "Chargement auto"
      ...
      function handleTouchStart(e) {
        ...
        // ============================================================
        // 1. PROTECTION UI (Boutons fixes & Panneau) - PRIORITÉ ABSOLUE
        // ============================================================

        // A. Boutons (Menu Burger, Caméra)
        if (e.target.closest("#openBtn") || e.target.closest("#cameraControls")) {
          return;
        }

        // B. LOGIQUE SWIPE (Panneau Latéral)
        // On vérifie ICI si l'utilisateur interagit avec le panneau AVANT de vérifier la 3D
        ...
        if (canStartOpening || canStartClosing) {
          ...
          // On retourne immédiatement. Le code ne lira jamais la partie Gizmo ci-dessous.
          return;
        }
        ...
        // ============================================================
        // Maintenant, on est sûr que l'utilisateur ne touche pas l'interface 2D.
        // On peut vérifier s'il touche le Gizmo 3D.
        if (typeof transformControl !== "undefined" && transformControl && selectedObjectInstance) {
          mouse.x = (touch.clientX / window.innerWidth) * 2 - 1;
          mouse.y = -(touch.clientY / window.innerHeight) * 2 + 1;
          raycaster.setFromCamera(mouse, camera);

          // On vérifie si on touche le Gizmo
          const gizmoIntersects = raycaster.intersectObjects(transformControl.children, true);
          if (gizmoIntersects.length > 0) {
            return; // On laisse Three.js gérer le déplacement de l'objet
          }
        }

        // ============================================================
        // 3. INITIALISATION MOUVEMENTS CAMÉRA
        // ============================================================
      ...
      function handleTouchMove(e) {
        ...
        // 3. Zoom et Panoramique (2 doigts)
          ...
          // --- C. Application du PAN (Déplacement) ---
          // On compare le milieu actuel avec le milieu précédent
          const panDX = currentMidX - previousMousePosition.x;
          const panDY = currentMidY - previousMousePosition.y;

          // On applique le déplacement si significatif
          ...
          if (Math.abs(diff) > 1) {
            ...
            // On met à jour la distance de référence pour éviter l'accélération exponentielle
            initialPinchDistance = currentPinchDistance;
          }

          // --- E. Mise à jour de la position de référence pour le prochain mouvement ---
      ...
      // Variable réutilisée pour éviter l'allocation mémoire à chaque frame
      ...
      function findObjectAndShowMenu(x, y) {
        ...
        if (typeof hideContextMenu === "function") {
          ...
          console.error("❌ Erreur : hideContextMenu n'est pas définie !");
          return;
        }
        ...
        if (intersection) {
          console.log("✅ 2. Objet touché !", intersection.object);

          if (intersection.object.userData && intersection.object.userData.instance) {
            console.log("✅ 3. Instance trouvée, lancement du menu...");
            populateAndShowContextMenu(x, y, intersection.object.userData.instance);
          } else {
            console.warn("⚠️ Objet touché mais pas de données 'instance' (userData vide ?)");
          }
        } else {
      ...
      function populateAndShowContextMenu(x, y, instance) {
        ...
        if (instance instanceof Point) {
          ...
          // -------------
          htmlContent += createAction(`removePoint(${index})`, "🗑️ Supprimer le point");
          ...
          htmlContent += createAction(`editPlane(${instance.id})`, "✏️ Modifier le plan");
          // --- AJOUT ---
          htmlContent += createAction(`openTransformationPanel('plane', ${instance.id})`, "🔄 Transformations");
          // -------------
          htmlContent += createAction(
          ...
          htmlContent += createAction(`removePlaneById(${instance.id})`, "🗑️ Supprimer");
        }

        // 3. Vérifier si du contenu a été généré
        if (htmlContent === "") {
          console.warn("⚠️ Menu vide : type d'objet non reconnu", instance);
          return;
        }
      ...
      function animate() {
        ...
        // Générer une signature simple de la position/rotation caméra
      ...
      // =====================================================================================
      // SECTION 4 : MISE À JOUR DE L'INTERFACE UTILISATEUR (UI)
      // =====================================================================================
  ...
  <div class="geometry-details-secondary">
    ...
    <div class="geometry-details-secondary">
      ...
      function clearAllPoints() {
        ...
        if (
          confirm(
            "Êtes-vous sûr de vouloir effacer tous les points ? (Les droites, plans et vecteurs existants seront conservés)"
          )
        ) {
          deselectCurrentObject();
      ...
      function addPlaneFromEquation() {
        ...
        // ========================================
        if (!equationStr) {
          showSplashScreen("⚠️ Veuillez entrer une équation (ex: 2x + y + z = 6).");
          return;
        }
        ...
        const b = getCoefficient("y"); // UI Y (Profondeur) → ThreeJS Z
        const c = getCoefficient("z"); // UI Z (Hauteur) → ThreeJS Y

        // Construction du vecteur normal Three.js
        // RAPPEL : a → x, c → y (hauteur), b → z (profondeur)
        const normal = new THREE.Vector3(a, c, b);

        if (normal.lengthSq() < 1e-8) {
          showSplashScreen("❌ Vecteur normal nul ou invalide. Vérifiez l'équation.");
          return;
        }
        ...
        // CAS B : Calcul automatique via la constante 'd' de l'équation
          ...
          try {
            if (constantStr) {
              // Évaluer l'expression mathématique restante (ex: "- 6" → -6)
        ...
        showSplashScreen(`✅ Plan "${uniqueName}" créé !`);
      }
      ...
      function updateVector() {
        ...
        if ([vx, vy_ui, vz_ui, ox, oy_ui, oz_ui].some((val) => isNaN(val))) {
          showSplashScreen("Toutes les coordonnées (Composantes et Origine) doivent être valides.");
          return;
        }
      ...
      function handleSelection(mesh) {
        ...
        // 1. Gestion du Toggle : Si on clique sur l'objet DÉJÀ sélectionné, on le désélectionne
        if (inst === selectedObjectInstance) {
          deselectCurrentObject();
          // On vide les formulaires pour être propre
        ...
        // 3. Attachement du Gizmo
        if (transformControl) {
          // CAS VECTEUR : On attache à l'arrowHelper (la flèche visuelle)
        ...
        // 4. Mise à jour du panneau latéral
        // Cette fonction va appeler editPoint, editLine, etc.
      ...
      function deselectCurrentObject() {
        ...
        // 3. Mettre à jour l'UI (c'est la partie cruciale)
      ...
      function calculateVectorSumFree() {
        // Nettoyer toute construction visuelle de Chasles qui pourrait être active
        clearChaslesConstruction();
      ...
      function calculateScalarProduct() {
        ...
        if (productLen > 1e-9) {
          ...
          angleInfo = `<br>
                 <span style="font-size:0.9em; color:#555;">
                    cos(θ) = <strong>${cosTheta.toFixed(4)}</strong><br>
                    Angle θ ≈ <strong>${angleDeg.toFixed(2)}°</strong>
                 </span>`;
        } else {
        ...
        r.innerHTML = `<strong>${v1.name} ⋅ ${v2.name} = ${parseFloat(dP.toFixed(4))}</strong>${angleInfo}`;
      }
      ...
      function calculateVectorProduct() {
        ...
        // --- NOUVELLE MISE EN FORME HTML (SIMPLE ET ALIGNÉE À GAUCHE) ---
      ...
      function calculateLineLineAngle() {
        ...
        if (a > 90) a = 180 - a;
        r.innerHTML = `Angle ≈ <strong>${a.toFixed(2)}°</strong>`;
      }
      ...
      function calculateLineLineIntersection() {
        ...
        if (Math.abs(mixedProduct) < 1e-6) {
          ...
          r.innerHTML = `Droites sécantes.<br>Point "${pointName}" créé.`;
          updateAllUI();
          saveState();
        } else {
          const distance = Math.abs(mixedProduct) / v1_cross_v2.length();
          r.innerHTML = `Droites non-coplanaires.<br>Distance min ≈ <strong>${distance.toFixed(3)}</strong>.`;
        }
      }

      // =====================================================================================
      // SECTION 8 : CONTRÔLE CAMÉRA ET FONCTIONS UTILITAIRES
      // =====================================================================================
      ...
      function panCamera(dX, dY) {
        // Vitesse dynamique : plus on est loin, plus ça bouge vite
      ...
      function applyRotation() {
        ...
        if (inst instanceof Point) {
          ...
          // Le vecteur lui-même (ses composantes) tourne aussi
      ...
      function applySymmetry() {
        ...
        // --- APPLICATION DE LA TRANSFORMATION À L'OBJET SÉLECTIONNÉ ---
        const inst = selectedObjectInstance;
      ...
      function saveScene() {
        ...
        if (!sceneName) {
          showSplashScreen("Veuillez donner un nom à votre scène.");
          return;
        }
        // 1. Sérialiser la géométrie actuelle
        const sceneData = {
          name: sceneName,
        ...
        // Vérifier si une scène avec le même nom existe déjà
        const existingSceneIndex = savedScenes.findIndex((s) => s.name === sceneName);
        if (existingSceneIndex > -1) {
          if (confirm(`Une scène nommée "${sceneName}" existe déjà. Voulez-vous la remplacer ?`)) {
        ...
        showSplashScreen(`✅ Scène "${sceneName}" enregistrée !`);

        // 4. Mettre à jour la liste des scènes chargeables
      ...
      function loadScene() {
        ...
        if (!sceneName) {
          showSplashScreen("Veuillez sélectionner une scène à charger.");
          return;
        }
        ...
        if (!sceneData) {
          showSplashScreen("Erreur : Impossible de trouver les données de la scène.");
          return;
        }
        ...
        sceneData.geometry.vectors.forEach((v) => {
          ...
          // On doit recréer le vecteur avec son ID pour garder la cohérence
        ...
        showSplashScreen(`✅ Scène "${sceneName}" chargée !`);
        // 3. Mettre à jour l'ensemble de l'interface
        updateAllUI();
      }
      ...
      function deleteScene() {
        const sceneName = document.getElementById("savedScenesSelect").value;
        if (!sceneName) {
          showSplashScreen("Veuillez sélectionner une scène à supprimer.");
          return;
        }

        if (confirm(`Êtes-vous sûr de vouloir supprimer la scène "${sceneName}" ? Cette action est irréversible.`)) {
          let savedScenes = JSON.parse(localStorage.getItem("savedScenes")) || [];
          const updatedScenes = savedScenes.filter((s) => s.name !== sceneName);
          localStorage.setItem("savedScenes", JSON.stringify(updatedScenes));
          populateSavedScenesSelect();
          showSplashScreen(`✅ Scène "${sceneName}"supprimée !`);
        }
      }
      ...
      function populateSavedScenesSelect() {
        ...
        if (savedScenes.length === 0) {
          select.add(new Option("Aucune scène enregistrée", ""));
        } else {
      ...
      function saveState() {
        ...
        // Cela empêche d'avoir [A, B, B] dans l'historique.
      ...
      function serializeSceneData() {
        return JSON.stringify({
          ...
          vectors: geometryManager.vectors.map((v) => ({
            ...
            // Sauvegarde des props spécifiques (normale de plan, etc)
      ...
      function handleHover(clientX, clientY) {
        ...
        if (
          ...
          // ... On cache l'info-bulle et on arrête tout.
        ...
        // ============================================================
        // Si on bouge la caméra, on cache tout pour ne pas gêner la vue
        if (isDragging || isPanning) {
          tooltip.style.display = "none";
          document.body.style.cursor = "default";
          return;
        ...
        if (intersection && intersection.object.userData.instance) {
          ...
          if (instance instanceof Point) {
            ...
            content = `↗️ <strong>${instance.name}</strong><span ${detailStyle}>Coord : (${x}, ${y_ui}, ${z_ui})</span><span ${detailStyle}>Norme : ${norm}</span>`;
      ...
      function syncDataFromGizmo() {
        ...
        // 3. DROITE (CORRIGÉ)
          ...
          // C. --- CORRECTION CRITIQUE : MISE À JOUR DU FORMULAIRE D'ÉDITION ---
      ...
      function finalizeGizmoMovement() {
        ...
        if (inst instanceof Vector) {
          ...
          // Sinon, Three.js essaie de mettre à jour un objet qui va être détruit.
          transformControl.detach();
        ...
        // --- CAS 2 : C'EST UNE DROITE ---
          ...
          // 2. Remettre le mesh à zéro (pour éviter que ça s'accumule)
      ...
      function updateLine() {
        ...
        // =========================================================
        // 5. MISE À JOUR DES DONNÉES ET DE LA SCÈNE (CRITIQUE)
        // =========================================================
      ...
      function exportSceneToJSON() {
        try {
          ...
          showSplashScreen("✅ Scène exportée avec métadonnées !");
        } catch (e) {
          console.error(e);
          showSplashScreen("❌ Erreur lors de l'export.");
        }
      }
      /**
      ...
      function importSceneFromJSON(inputElement) {
        ...
        reader.onload = function (e) {
          try {
            ...
            showSplashScreen(`✅ Scène "${file.name}" importée !`);
          } catch (err) {
            console.error(err);
            showSplashScreen("❌ Erreur : Fichier invalide ou corrompu.");
          }
    ...
    `;
      ...
      // --- LOGIQUE DE MISE À JOUR DEPUIS LE TABLEUR (POINTS) ---
    ...
    `;
      ...
      function sheetUpdateVector(id) {
        ...
        // Récupération ligne par ID pour être sûr (ou par element relatif si on passait 'this')
      ...
      const SNAP_ROTATE = THREE.MathUtils.degToRad(15); // Rotation toujours fixée à 15°
